...
import { decodePolyline, pacePerKm, effortScore, estimateOneRepMax, DISTANCE_SPORTS } from '@forge/contracts';
...
import {
  formatDistance, formatDuration, formatDate, formatTime, formatPace,
  formatElevation, formatLoadG, SPORT_LABEL,
} from '@/lib/format';
    ...
    <article className="space-y-8">
      <header>
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <Badge>{activity.visibility === 'private' ? 'Only me'
            : activity.visibility === 'followers' ? 'Followers' : 'Public'}</Badge>
        </div>
        <h1 className="mt-4 text-page-title font-display text-bone-100">{activity.title || 'Untitled activity'}</h1>
        <p className="mt-2 text-secondary muted">
          {activity.athlete.displayName} · {formatDate(activity.startedAt)} at {formatTime(activity.startedAt)}
        </p>
      ...
      <section aria-labelledby="stats-heading">
        <h2 id="stats-heading" className="sr-only">Activity statistics</h2>
        <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-5">
          {isDistance && <Card><Metric label="Distance" value={formatDistance(activity.distanceM, units)} size="xl" /></Card>}
          <Card><Metric label="Moving time" value={formatDuration(activity.movingS)} size="xl" /></Card>
          {isDistance && pace !== null && <Card><Metric label="Pace" value={formatPace(pace, units)} size="xl" /></Card>}
          {activity.elevationGainM > 0 && (
            <Card><Metric label="Climbing" value={formatElevation(activity.elevationGainM, units)} size="xl" /></Card>
          )}
          {activity.avgHr && <Card><Metric label="Avg heart rate" value={activity.avgHr} unit="bpm" size="xl" /></Card>}
          {effort !== null && (
            <Card><Metric label="Effort" value={`${effort}/10`} size="xl" hint="From heart rate against your max" /></Card>
          )}
        </div>
      </section>
      ...
      {splits.length > 0 && (
        <section aria-labelledby="splits-heading">
          <h2 id="splits-heading" className="eyebrow mb-4">Splits</h2>
            ...
            <table className="w-full min-w-[420px] border-collapse text-secondary">
              ...
              <thead>
                <tr className="border-b border-ink-600 text-left">
                  <th scope="col" className="px-4 py-3 eyebrow font-normal">Km</th>
                  <th scope="col" className="px-4 py-3 eyebrow font-normal">Time</th>
                  <th scope="col" className="px-4 py-3 eyebrow font-normal">Pace</th>
                  <th scope="col" className="px-4 py-3 eyebrow font-normal">Climb</th>
                  <th scope="col" className="px-4 py-3 eyebrow font-normal">HR</th>
                </tr>
              </thead>
              <tbody>
                {splits.map((split) => (
                  <tr key={split.idx} className="border-b border-ink-600 last:border-0">
                    ...
                    <td className="px-4 py-3 tabular-nums">{split.elevationM !== null ? `${Math.round(split.elevationM)} m` : '—'}</td>
      ...
      {Object.keys(byExercise).length > 0 && (
        <section aria-labelledby="sets-heading">
          <h2 id="sets-heading" className="eyebrow mb-4">Sets</h2>
          <div className="space-y-4">
            {Object.entries(byExercise).map(([slug, exerciseSets]) => {
              const best = exerciseSets.reduce((top, s) =>
                (estimateOneRepMax(s.loadG, s.reps) ?? 0) > (estimateOneRepMax(top.loadG, top.reps) ?? 0) ? s : top);
              const oneRm = estimateOneRepMax(best.loadG, best.reps);
              return (
                <Card key={slug}>
                  <div className="flex flex-wrap items-baseline justify-between gap-3">
                    <h3 className="text-card-title capitalize text-bone-100">{slug.replace(/-/g, ' ')}</h3>
                    {oneRm !== null && (
                      <p className="text-secondary muted">
                        Estimated 1RM <span className="font-semibold tabular-nums text-bone-100">{formatLoadG(oneRm, units)}</span>
                      </p>
                    )}
                  </div>
      ...
      <p className="text-secondary">
        <Link href="/activities" className="font-semibold text-signal hover:underline underline-offset-4">
          ← All activities
        </Link>
      </p>
    </article>
