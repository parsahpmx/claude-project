...
const VARIANTS: Record<Variant, string> = {
  ...
  ghost:
    'bg-transparent text-current border border-current/25 hover:border-current/50 hover:bg-current/[0.06]',
  ...
  return (
    <As
      className={clsx(
        ...
        padded && 'p-5',
        interactive &&
          'transition-transform duration-200 ease-forge hover:-translate-y-0.5 hover:shadow-lift',
        className,
      )}
    >
      {children}
...
export function Metric({
  ...
  return (
    <div>
      <p
        className={clsx(
          'tabular-nums text-bone-100',
          size === 'xl' ? 'text-metric-xl' : 'text-metric-l',
        )}
      >
        {value}
...
export function Badge({
  ...
  return (
    <span
      className={clsx(
        'inline-flex items-center gap-1.5 rounded-pill border px-2.5 py-1 text-caption font-semibold whitespace-nowrap',
        tones[tone],
      )}
    >
      {children}
...
export function ErrorState({
  ...
  return (
    <div
      role="alert"
      className="rounded-card border border-state-bad/30 bg-state-bad/[0.06] px-6 py-7"
    >
...
export function Skeleton({ className }: { className?: string }) {
  return (
    <div aria-hidden className={clsx('animate-pulse rounded-control bg-ink-700', className)} />
  );
}
