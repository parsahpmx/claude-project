...
const WORKOUTS: Workout[] = [
  ...
  },
  {
    ...
    intensity: 'low',
    imageKey: 'workout-row',
  },
  {
    id: 'w12',
    title: 'Post-Session Reset',
    style: 'recovery',
    minutes: 15,
    level: 'beginner',
    coach: 'Inés',
...
export function WorkoutDiscovery() {
  ...
  const results = useMemo(() => {
    ...
    return WORKOUTS.filter((workout) => {
      if (query && !workout.title.toLowerCase().includes(query) && !workout.style.includes(query))
        return false;
      ...
      if (equipment.length > 0 && !workout.equipment.every((item) => equipment.includes(item)))
        return false;
