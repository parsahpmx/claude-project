...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center rounded-card border border-dashed border-ink-900/15 px-6 py-14 text-center">
      ...
      <p className="mt-2 max-w-sm text-sm leading-relaxed opacity-65">{body}</p>
...
export function ErrorState({ title, body, action }: { title: string; body: string; action?: ReactNode }) {
  return (
    ...
    >
      <p className="eyebrow text-signal-bad">Something went wrong</p>
      <h3 className="display mt-2 text-lg">{title}</h3>
      <p className="mt-2 max-w-prose text-sm leading-relaxed opacity-75">{body}</p>
...
export function SuccessState({ title, body, action }: { title: string; body: string; action?: ReactNode }) {
  return (
    <div className="rounded-card border border-signal-good/30 bg-signal-good/[0.07] px-6 py-8">
      <p className="eyebrow text-signal-good">Done</p>
      <h3 className="display mt-2 text-lg">{title}</h3>
      <p className="mt-2 max-w-prose text-sm leading-relaxed opacity-75">{body}</p>
      {action && <div className="mt-5">{action}</div>}
    </div>
...
export function SkeletonCard() {
  return (
    <div className="rounded-card border border-ink-900/10 bg-bone-100 p-6">
...
export function Status({
  ...
  const map = {
    completed: { label: 'Completed', glyph: '✓', tone: 'text-signal-good border-signal-good/30 bg-signal-good/10' },
    scheduled: { label: 'Scheduled', glyph: '○', tone: 'text-smoke-500 border-ink-900/15 bg-ink-900/[0.04]' },
    skipped: { label: 'Missed', glyph: '×', tone: 'text-signal-bad border-signal-bad/30 bg-signal-bad/[0.08]' },
    connected: { label: 'Connected', glyph: '✓', tone: 'text-signal-good border-signal-good/30 bg-signal-good/10' },
    'not-connected': { label: 'Not connected', glyph: '○', tone: 'text-smoke-500 border-ink-900/15 bg-ink-900/[0.04]' },
    syncing: { label: 'Syncing', glyph: '↻', tone: 'text-signal-info border-signal-info/30 bg-signal-info/10' },
    pending: { label: 'Pending', glyph: '•', tone: 'text-signal-warn border-signal-warn/30 bg-signal-warn/10' },
    paid: { label: 'Paid', glyph: '✓', tone: 'text-signal-good border-signal-good/30 bg-signal-good/10' },
  } as const;
  ...
  return (
    <span
      className={clsx(
        ...
        earned
          ? 'border-ember/30 bg-ember/10 text-ember-600'
          : 'border-ink-900/12 bg-ink-900/[0.03] text-smoke-500',
      )}
    >
