...
export const metadata = {
  title: 'Nutrition',
  description:
    'Targets calculated from your own physiology, with recipes and a weekly shopping list.',
};
...
const SECTIONS = [
  ...
  {
    name: 'Protein',
    items: ['Chicken breast 800g', 'Sirloin steak 300g', 'Eggs ×12', 'Salmon fillet 300g'],
  },
  { name: 'Dairy', items: ['Greek yoghurt 650g', 'Cottage cheese 200g', 'Oat milk 1L'] },
  {
    name: 'Pantry',
    items: ['Rolled oats 400g', 'Basmati rice 300g', 'Whey protein 200g', 'Peanut butter'],
  },
    ...
    <>
        ...
        <div className="grid gap-12 pt-16 lg:grid-cols-[1.2fr_1fr] lg:items-center">
          <div>
            ...
            <h1 className="display text-display-lg text-balance">
              EAT FOR THE TRAINING YOU ACTUALLY DID.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-bone-200/70">
              Targets calculated from your height, weight, age and the sessions on your plan — not a
              slider you guessed at. Recipes that match how you eat, and a shopping list that writes
              itself.
            </p>
            <div className="mt-9 flex flex-wrap gap-3">
              ...
              <ButtonLink href="#recipes" variant="inverse" size="lg">
                Browse Recipes
              </ButtonLink>
            </div>
          </div>
          ...
          <Card tone="dark">
            ...
            <ul className="space-y-3 text-sm">
              {[
                ...
                <li
                  key={slot}
                  className="flex items-center justify-between gap-4 border-b border-bone-200/10 pb-3 last:border-0"
                >
                  <div>
      ...
      <Section tone="light" size="md">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
          {[
            [
              '01',
              'Resting rate',
              'Calculated from height, weight and age — the energy you burn doing nothing.',
            ],
            [
              '02',
              'Activity',
              'Multiplied by the sessions your plan actually schedules, not a self-reported level.',
            ],
            [
              '03',
              'Goal adjustment',
              'Capped at ±20%, with a hard floor at 1,500 kcal and never below your resting rate.',
            ],
            [
              '04',
              'Macro split',
              'Protein by bodyweight and goal, fat floored for hormonal health, carbs fill the rest.',
            ],
          ].map(([step, title, body]) => (
            <Card key={step}>
        ...
        <p className="mt-8 max-w-prose text-sm leading-relaxed text-muted">
          The floor is not a detail. An unbounded deficit is the one place a fitness product can do
          real harm, so it is enforced in the domain layer where every client — web, mobile and
          coach — hits the same rule.
        </p>
      </Section>
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <h2 className="display text-display-md text-balance">
              YOUR WEEK, AGGREGATED BY AISLE.
            </h2>
            <p className="mt-6 max-w-prose leading-relaxed text-muted">
              Plan the week and FORGE merges every ingredient across every recipe into one list,
              sorted the way a shop is laid out. Lines only merge when the units match — 200g of
              tomatoes and 2 tomatoes stay separate, because an ugly list is recoverable and a wrong
              one sends you home short.
            </p>
            <div className="mt-8">
              <ButtonLink href="/assessment" size="lg">
                Build My Week
                  ...
                  <ul className="space-y-3">
                    {section.items.map((item, index) => (
                      <li key={item} className="flex items-center gap-3 text-sm">
                        ...
                        <span className={index === 0 ? 'text-muted line-through' : 'opacity-80'}>
                          {item}
                        </span>
                      </li>
                    ))}
      ...
      <Section tone="dark" size="md">
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          ...
          <div>
            <h2 className="display text-display-md text-balance">
              SWAP ANY MEAL. THE MATHS STILL WORKS.
            </h2>
            <p className="mt-6 leading-relaxed text-bone-200/70">
              Do not fancy the dinner? Swap it. FORGE only offers alternatives that fit your diet
              preference and lands the day within a few grams of your targets. Log something
              off-plan and the remaining meals adjust rather than the day being written off.
            </p>
            <ul className="mt-8 space-y-3">
              {['Swap meal', 'Save to favourites', 'Add to shopping list', 'Log meal'].map(
                (action) => (
