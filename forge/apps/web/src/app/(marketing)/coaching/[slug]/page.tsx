    ...
    <>
            ...
            <div>
                ...
                <Stat
                  ...
                  value={
                    coach.availableSlotsThisWeek > 0
                      ? `${coach.availableSlotsThisWeek} slots`
                      : 'Waitlist'
                  }
                  hint="this week"
                  tone="dark"
              ...
              <div className="mt-7 flex flex-wrap gap-2">
                {coach.specialties.map((specialty) => (
                  <Chip key={specialty} tone="inverse">
                    {specialty.replace(/-/g, ' ')}
                  </Chip>
                ))}
              </div>
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <ButtonLink href={`/signup?coach=${coach.slug}`} size="lg">
                  Choose As My Coach
                </ButtonLink>
                <ButtonLink
                  href={`/signup?coach=${coach.slug}&booking=consultation`}
                  variant="inverse"
                  size="lg"
                >
          ...
          <div className="space-y-12">
            ...
            <div>
              ...
              <blockquote className="border-l-2 border-ember pl-6">
                <p className="max-w-prose text-lg italic leading-relaxed opacity-85">
                  {coach.philosophy}
                </p>
              </blockquote>
            </div>
            ...
            <div>
              ...
              <ul className="grid gap-3 sm:grid-cols-2">
                {coach.certifications.map((certification) => (
                  <li
                    key={certification}
                    className="light-surface flex items-center gap-3 rounded-[8px] border border-ink-900/10 bg-bone-100 px-4 py-3 text-sm"
                  >
                    <span aria-hidden className="text-accent">
                      ✓
                    </span>
            ...
            <Card>
              ...
              <ul className="space-y-4">
                <PriceRow
                  ...
                  price={
                    coach.consultationPriceCents === 0
                      ? 'Free'
                      : formatCents(coach.consultationPriceCents)
                  }
                  note="No commitment"
                />
                <PriceRow
                  label="60-minute session"
                  price={formatCents(coach.sessionPriceCents)}
                  note="One-off"
                />
                <PriceRow
              ...
              <div className="mt-6">
                <ButtonLink href={`/signup?coach=${coach.slug}`} block size="lg">
                  Book Session
                </ButtonLink>
              </div>
              <p className="mt-3 text-center text-xs text-muted">
                Cancel or change coach at any time.
              </p>
            </Card>
            ...
            <Card>
              ...
              {coach.availableSlotsThisWeek > 0 ? (
                <div className="grid grid-cols-2 gap-2">
                  {SLOTS.slice(0, Math.max(2, coach.availableSlotsThisWeek * 2)).map(
                    (slot, index) => (
                      <span
                        key={slot}
      ...
      {reviews.length > 0 && (
        <Section tone="light" size="md">
          <SectionHeading
            eyebrow="Reviews"
            title={`${formatNumber(coach.reviewCount)} MEMBER REVIEWS.`}
          />
      ...
      {programs.length > 0 && (
        <Section tone="bone" size="md">
          <SectionHeading
            eyebrow="Programmes"
            title={`PROGRAMS BY ${coach.firstName.toUpperCase()}.`}
          />
...
function PriceRow({
  ...
  return (
    <li
      className={`flex items-start justify-between gap-4 rounded-[8px] px-4 py-3.5 ${highlight ? 'accent-tint bg-ember/[0.07] ring-1 ring-ember/20' : 'bg-ink-900/[0.03]'}`}
    >
      <div>
