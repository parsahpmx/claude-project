        ...
        {bookings.length === 0 ? (
          ...
          <div className="space-y-8">
            {[...byDay.entries()].map(([day, entries]) => (
              <section key={day}>
                <h2 className="eyebrow mb-4">{formatLongDate(day)}</h2>
                  ...
                  {entries.map((entry) => (
                    <Card key={entry.booking.id}>
                      ...
                      <p className="mt-3 text-xs capitalize text-muted">
                        ...
                        {entry.booking.priceCents > 0 &&
                          ` · ${formatCents(entry.booking.priceCents)}`}
                      </p>
      ...
      <section className="mt-12">
        <Card tone="dark">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
            <div className="dark-surface grain relative aspect-video overflow-hidden rounded-card bg-ink-800">
              ...
              <div className="absolute bottom-4 right-4 aspect-video w-1/4 rounded-[8px] border border-bone-200/20 bg-ink-900/80">
                <div className="grid h-full place-items-center text-[0.625rem] text-muted">
                  Client
                </div>
              </div>
                ...
                {['Mute', 'Camera', 'Share', 'End'].map((label) => (
                  <span
                    ...
                    className={`rounded-pill px-4 py-2 text-[0.6875rem] uppercase tracking-[0.1em] ${
                      label === 'End'
                        ? 'bg-signal-bad text-bone-100'
                        : 'bg-ink-900/80 text-bone-200/80'
                    }`}
                  >
                    {label}
                  </span>
              ...
              <div>
                ...
                <dl className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <dd className="tabular-nums text-bone-100">92%</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-muted">Streak</dt>
                    <dd className="tabular-nums text-bone-100">18d</dd>
                  </div>
                </dl>
              </div>
            </div>
          ...
          <p className="mt-5 text-xs text-muted">
            Video calling is not wired up in this prototype — the session layout, agenda and metric
            panel are.
          </p>
        </Card>
      </section>
